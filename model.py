...
class EpiDenoise20(nn.Module):
    def __init__(self, 
        ...
        self.conv1 = ConvTower(input_dim, int((0.75 * (d_model // (1.5**n_cnn_layer)))), kernel_size, stride, dilation)
        self.convm = ConvTower(input_dim, int((0.25 * (d_model // (1.5**n_cnn_layer)))), 1, stride, dilation)
...
def train_epidenoise20(hyper_parameters, checkpoint_path=None, start_ds=0):
    ...
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=110, gamma=0.8)
