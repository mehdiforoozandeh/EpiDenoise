...
class EVAL_CANDI(object):
    ...
    def saga(self, bios_name, x_dsf, fill_in_y_prompt=False, n_components=18, n_iter=100, tol=1e-4, random_state=0, resolution=200):
        ...
        print("obs_MAP", {k: float(v)/len(obs_data.shape[0]) for k, v in Counter(SAGA_obs_MAP).items()})
        ...
        print("den_MAP", {k: float(v)/len(obs_data.shape[0]) for k, v in Counter(SAGA_den_MAP).items()})
        ...
        print("denimp_MAP", {k: float(v)/len(obs_data.shape[0]) for k, v in Counter(SAGA_denimp_MAP).items()})
        ...
        print("latent_MAP", {k: float(v)/len(obs_data.shape[0]) for k, v in Counter(SAGA_latent_MAP).items()})
