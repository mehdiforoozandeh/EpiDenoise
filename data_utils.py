...
class BAM_TO_SIGNAL(object):
    def __init__(self, bam_file, chr_sizes_file, resolution=25, output_suffix=""):
        self.bam_file = bam_file
        self.chr_sizes_file = chr_sizes_file
        self.resolution = resolution
        self.output_suffix = output_suffix  # Suffix for output directories (e.g., "_control")
        self.read_chr_sizes()
        self.load_bam()
    ...
    def save_signal_metadata(self, depth, coverage, downsampling_factor):
        output_dir = f"{'/'.join(self.bam_file.split('/')[:-1])}/signal_DSF{downsampling_factor}_res{self.resolution}{self.output_suffix}/"
        if os.path.exists(output_dir) == False:
            os.mkdir(output_dir)
        
        filename = f"{output_dir}metadata.json"
        mdict = {
            "coverage":coverage,
            "depth":depth,
